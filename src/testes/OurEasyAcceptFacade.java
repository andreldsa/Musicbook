...
public class OurEasyAcceptFacade {
	...
	public String getAtributoUsuario(String login, String atributo) throws LoginInvalidoException, UsuarioInexistenteException, AtributoInvalidoException, AtributoInexistenteException{
		int userId = sistema.getUsuarioID(login);
		if(userId == -1) throw new UsuarioInexistenteException("Usuário inexistente");
		if(atributo == null || atributo.isEmpty()) throw new AtributoInvalidoException("Atributo inválido");
	...
	public String getPerfilMusical(int idSessao) throws SessaoInexistenteException{
		return sistema.getPerfilMusical(idSessao).toString().replace('[', '{').replace(']', '}').replace(" ","");
	}

	public int postarSom(String idSessao, String link, String dataCriacao) throws SessaoInexistenteException, SomInvalidoException, DataDeCriacaoInvalidaException, SessaoInvalidaException, DataDeCriacaoInvalida{
		...
		} catch (Exception e) { throw new DataDeCriacaoInvalida(e);}
		return sistema.postarSom(idSessaoint, link, dataCriacao);
	}

	public int postarSom(int idSessao, String link, String dataCriacao) throws SessaoInexistenteException, SomInvalidoException, DataDeCriacaoInvalidaException, SessaoInvalidaException, DataDeCriacaoInvalida{
		return this.postarSom(idSessao + "", link, dataCriacao);
	}


	public String getAtributoSom(String idSom, String atributo) throws AtributoInvalidoException, AtributoInexistenteException, SomInvalidoException{
		if(idSom== null || idSom.isEmpty()) throw new SomInvalidoException("Som inválido");
	...
	public void seguirUsuario(String idSessao, String login) throws LoginInvalidoException, SessaoInexistenteException, UsuarioInexistenteException, SessaoInvalidaException {
		int idSessaoint = idSessaoCheck(idSessao);
	...
	public String getListaDeSeguidores(String idSessao) throws SessaoInexistenteException, SessaoInvalidaException {
		int idSessaoint = idSessaoCheck(idSessao);
		return sistema.getListaDeSeguidores(idSessaoint).toString().replace('[', '{').replace(']', '}').replace(" ","");
	}
	...
	public String getFonteDeSons(String idSessao) throws SessaoInexistenteException, SessaoInvalidaException {
		int idSessaoint = idSessaoCheck(idSessao);
		return sistema.getFonteDeSons(idSessaoint).toString().replace('[', '{').replace(']', '}').replace(" ","");
	}

	public String getSonsFavoritos(String idSessao) throws SessaoInexistenteException, SessaoInvalidaException {
		int idSessaoint = idSessaoCheck(idSessao);
		return sistema.getSonsFavoritos(idSessaoint).toString().replace('[', '{').replace(']', '}').replace(" ","");
	}

	public String getFeedExtra(String idSessao) throws SessaoInexistenteException, SessaoInvalidaException {
		int idSessaoint = idSessaoCheck(idSessao);
		return sistema.getFeedExtra(idSessaoint).toString().replace('[', '{').replace(']', '}').replace(" ","");
	}

	public void favoritarSom(String idSessao, int idSom) throws SessaoInexistenteException, SomInexistenteException, SessaoInvalidaException {
		int idSessaoint = idSessaoCheck(idSessao);
		sistema.favoritarSom(idSessaoint, idSom);
	}

	public void favoritarSom(String idSessao, String idSom) throws SessaoInexistenteException, SomInvalidoException, SomInexistenteException, SessaoInvalidaException {
		int idSessaoint = idSessaoCheck(idSessao);
		if(idSom == null || idSom.isEmpty()) throw new SomInvalidoException("Som inválido");
	...
	public String getMainFeed(String idSessao) throws SessaoInexistenteException, SessaoInvalidaException {
		int idSessaoint = idSessaoCheck(idSessao);
		return sistema.getMainFeed(idSessaoint).toString().replace('[', '{').replace(']', '}').replace(" ","");
	}

	public void setMainFeedRule(String idSessao, String rule) throws SessaoInexistenteException, SessaoInvalidaException, RegraDeComposicaoInexistenteException, RegraDeComposicaoInvalidaException {
		int idSessaoint = idSessaoCheck(idSessao);
		if(rule == null || rule.isEmpty()) throw new RegraDeComposicaoInvalidaException("Regra de composição inválida");
		...
		else if(rule.equals("PRIMEIRO SONS DE FONTES DAS QUAIS FAVORITEI SONS NO PASSADO")){
			sistema.setMainFeedRule(CompositionRule.Rule3, idSessaoint);
		}else{
			throw new RegraDeComposicaoInexistenteException("Regra de composição inexistente");
		}
	}

	public void setMainFeedRule(int idSessao, String rule) throws SessaoInexistenteException, RegraDeComposicaoInexistenteException, RegraDeComposicaoInvalidaException {
		if(rule == null || rule.isEmpty()) throw new RegraDeComposicaoInvalidaException("Regra de composição inválida");
		if(rule.equals("PRIMEIRO OS SONS POSTADOS MAIS RECENTEMENTE PELAS FONTES DE SONS")){
			sistema.setMainFeedRule(CompositionRule.Rule1, idSessao);
		}
		else if(rule.equals("PRIMEIRO OS SONS COM MAIS FAVORITOS")){
			sistema.setMainFeedRule(CompositionRule.Rule2, idSessao);
		}
		else if(rule.equals( "PRIMEIRO SONS DE FONTES DAS QUAIS FAVORITEI SONS NO PASSADO")){
			sistema.setMainFeedRule(CompositionRule.Rule3, idSessao);
		}else{
			throw new RegraDeComposicaoInexistenteException("Regra de composição inexistente");
		}
	}
	...
	private int idSessaoCheck(String idSessao) throws SessaoInvalidaException, SessaoInexistenteException{
	...
	public void encerrarSessao(String login) throws LoginInvalidoException, SessaoInexistenteException{
	...
	public int getIDUsuario(String idSessao) throws SessaoInexistenteException, SessaoInvalidaException{
	...
	public String getFontesDeSons(String idSessao) throws SessaoInexistenteException, SessaoInvalidaException{
		return sistema.getFonteDeSons(idSessaoCheck(idSessao)).toString().replace('[', '{').replace(']', '}').replace(" ","");
	}
	public String getVisaoDosSons(String idSessao) throws SessaoInexistenteException, SessaoInvalidaException{
		return sistema.getVisaoDosSons(idSessaoCheck(idSessao)).toString().replace('[', '{').replace(']', '}').replace(" ","");
	}
