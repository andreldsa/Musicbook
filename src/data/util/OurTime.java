...
private Calendar cal;
	 ...
	 */
	public OurTime(int dia, int mes, int ano, int hora, int minuto, int segundo) throws DiaInvalidoException, MesInvalidoException, AnoInvalidoException {
		...
		if(teste.get(Calendar.YEAR) > ano)
			...
			if((teste.get(Calendar.YEAR) == ano) && (teste.get(Calendar.MONTH) > mes))
				throw new MesInvalidoException("Data de Criação inválida");
			else
				if((teste.get(Calendar.YEAR) == ano) && (teste.get(Calendar.MONTH) == mes) && (teste.get(Calendar.DAY_OF_MONTH) > dia))
					throw new DiaInvalidoException("Data de Criação inválida");
		this.cal = new GregorianCalendar(ano,mes-1,dia,hora,minuto,segundo);
